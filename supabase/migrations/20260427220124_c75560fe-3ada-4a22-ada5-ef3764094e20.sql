...
BEGIN
  ...
  INSERT INTO public.rsvps (event_id, user_id, status, checked_in_at, cancelled_at) VALUES
    (ev_dinner, lydia,'going',NULL,NULL),(ev_dinner, nabil,'going',NULL,NULL),(ev_dinner, lauren,'going',NULL,NULL),
    (ev_dinner, sara,'maybe',NULL,NULL),(ev_dinner, alex,'going',NULL,NULL),(ev_dinner, maya,'maybe',NULL,NULL),
    (ev_dinner, jordan,'no',NULL,NULL),(ev_dinner, sofia,'going',NULL,NULL),
    (ev_volley, nabil,'going',NULL,NULL),(ev_volley, jordan,'maybe',NULL,NULL),(ev_volley, sofia,'going',NULL,NULL),
    (ev_volley, alex,'maybe',NULL,NULL),(ev_volley, sara,'no',NULL,NULL),(ev_volley, lydia,'going',NULL,NULL),(ev_volley, maya,'no',NULL,NULL),
    (ev_potluck, lydia,'going',NULL,NULL),(ev_potluck, lauren,'going',NULL,NULL),(ev_potluck, maya,'maybe',NULL,NULL),
    (ev_potluck, sofia,'no',NULL,NULL),(ev_potluck, jordan,'no',NULL,NULL),(ev_potluck, alex,'going',NULL,NULL),
    (ev_potluck, nabil,'going',NULL,NULL),(ev_potluck, sara,'maybe',NULL,NULL),
    (ev_picnic, lydia,'going',picnic_at + interval '15 minutes',NULL),(ev_picnic, lauren,'going',picnic_at + interval '20 minutes',NULL),
    (ev_picnic, sara,'going',NULL,picnic_at - interval '2 hours'),(ev_picnic, maya,'no',NULL,NULL),
    (ev_picnic, sofia,'going',picnic_at + interval '30 minutes',NULL),(ev_picnic, nabil,'going',picnic_at + interval '10 minutes',NULL),
    (ev_picnic, alex,'maybe',NULL,NULL),(ev_picnic, jordan,'no',NULL,NULL),
    (ev_brunch, nabil,'going',brunch_at + interval '5 minutes',NULL),(ev_brunch, alex,'going',brunch_at + interval '25 minutes',NULL),
    (ev_brunch, jordan,'no',NULL,NULL),(ev_brunch, lydia,'going',brunch_at + interval '10 minutes',NULL),
    (ev_brunch, lauren,'maybe',NULL,NULL),(ev_brunch, sara,'no',NULL,NULL),(ev_brunch, maya,'maybe',NULL,NULL),
    (ev_brunch, sofia,'going',brunch_at + interval '40 minutes',NULL),
    (ev_japan, lydia,'going',NULL,NULL),(ev_japan, nabil,'going',NULL,NULL),(ev_japan, lauren,'maybe',NULL,NULL),
    (ev_japan, sara,'no',NULL,NULL),(ev_japan, maya,'maybe',NULL,NULL),(ev_japan, alex,'maybe',NULL,NULL),
    (ev_japan, jordan,'no',NULL,NULL),(ev_japan, sofia,'maybe',NULL,NULL)
  ON CONFLICT DO NOTHING;
  ...
  -- Rich message history across all event chats
  INSERT INTO public.messages (conversation_id, sender_id, body, created_at) VALUES
    -- Sunset Rooftop Dinner
    (conv_dinner, me_id,  'Rooftop dinner this Friday! Reservation locked in for 8', now() - interval '5 days'),
    (conv_dinner, alex,   'Is there parking nearby or should we Uber?',              now() - interval '4 days'),
    (conv_dinner, me_id,  'Definitely Uber, parking is brutal in 14th',              now() - interval '4 days' + interval '10 minutes'),
    (conv_dinner, lauren, 'Dress code? Are we doing fancy or casual?',               now() - interval '3 days'),
    (conv_dinner, me_id,  'Smart casual. Bring a layer, gets chilly up there 🌬️',   now() - interval '3 days' + interval '5 minutes'),
    (conv_dinner, lydia,  'Cannot wait!! 🌅',                                        now() - interval '2 hours'),
    (conv_dinner, nabil,  'Playlist incoming 🎶',                                    now() - interval '90 minutes'),
    -- Beach Volleyball
    (conv_volley, me_id,  'Volleyball Saturday at East Potomac! Who''s in?',         now() - interval '4 days'),
    (conv_volley, sofia,  'In!! I''ll bring snacks and gatorade',                    now() - interval '4 days' + interval '10 minutes'),
    (conv_volley, nabil,  'Down. What time again?',                                  now() - interval '3 days'),
    (conv_volley, me_id,  '11am — voted on it. Court reservation pending',           now() - interval '3 days' + interval '5 minutes'),
    (conv_volley, jordan, 'I got the net 🏐',                                        now() - interval '1 day'),
    (conv_volley, jordan, 'Bringing extra sunscreen, last time we all burned 😅',    now() - interval '12 hours'),
    -- Friendsgiving Potluck
    (conv_potluck, me_id,  'Friendsgiving planning time!! 🦃',                       now() - interval '10 days'),
    (conv_potluck, lauren, 'I''ll handle dessert — pumpkin pie + apple crumble?',    now() - interval '10 days' + interval '30 minutes'),
    (conv_potluck, maya,   'MAC & CHEESE incoming 🧀🧀',                             now() - interval '9 days'),
    (conv_potluck, lydia,  'I''ll bring wine and cranberry sauce',                   now() - interval '8 days'),
    (conv_potluck, alex,   'Mashed potatoes and gravy — got it 🥔',                  now() - interval '6 days'),
    (conv_potluck, me_id,  'Turkey ordered from Whole Foods, picking up Wed',        now() - interval '3 days'),
    (conv_potluck, lauren, 'Ryan you''re a legend 🙌',                               now() - interval '3 days' + interval '20 minutes'),
    -- Cherry Blossom Picnic
    (conv_picnic, me_id,  'Hey everyone! Picnic is on for next month 🌸',           now() - interval '7 days'),
    (conv_picnic, lydia,  'Yesss I''ve been waiting all year for this',              now() - interval '7 days' + interval '15 minutes'),
    (conv_picnic, lauren, 'I can bring strawberries and a charcuterie board 🧀',     now() - interval '6 days'),
    (conv_picnic, sara,   'Sorry y''all I have a midterm that day 😭',               now() - interval '5 days'),
    (conv_picnic, maya,   'I''ll be out of town that weekend, next time!',           now() - interval '4 days'),
    (conv_picnic, lydia,  'I got the picnic blanket + snacks covered ✅',            now() - interval '2 days'),
    (conv_picnic, me_id,  'Meeting at the Tidal Basin entrance at 2pm sharp',        now() - interval '1 day'),
    -- Spring Brunch
    (conv_brunch, me_id,  'Brunch after midterms!! Founding Farmers 11am',           now() - interval '8 days'),
    (conv_brunch, nabil,  'MIMOSAS 🥂',                                              now() - interval '8 days' + interval '5 minutes'),
    (conv_brunch, lydia,  'I made the reservation for 6 under my name',              now() - interval '7 days'),
    (conv_brunch, alex,   'Pancakes >>> waffles, fight me',                          now() - interval '5 days'),
    (conv_brunch, jordan, 'Can''t make it sadly, family thing — have fun!',          now() - interval '4 days'),
    (conv_brunch, nabil,  'I''ll pick up the tab, we can split via the budget tab',  now() - interval '2 days'),
    -- Tokyo Trip
    (conv_japan, me_id,  'JAPAN PLANNING THREAD 🇯🇵🍜 60 days out!',                 now() - interval '14 days'),
    (conv_japan, lydia,  'I CANNOT WAIT. Already learning hiragana 📚',              now() - interval '14 days' + interval '20 minutes'),
    (conv_japan, nabil,  'Putting together a list of must-eat ramen spots',          now() - interval '13 days'),
    ...
    (conv_japan, maya,   'I''m a maybe — waiting on work to confirm PTO',            now() - interval '10 days'),
    (conv_japan, alex,   'Same boat — fingers crossed 🤞',                           now() - interval '10 days' + interval '30 minutes'),
    (conv_japan, lauren, 'teamLab tickets sell out FAST btw, should book soon',      now() - interval '6 days'),
    (conv_japan, me_id,  'On it — booking teamLab + Ghibli museum this week',        now() - interval '6 days' + interval '15 minutes'),
    (conv_japan, nabil,  'Day trip to Mt. Fuji — bullet train or bus?',              now() - interval '4 days'),
    (conv_japan, lydia,  'Bullet train all the way 🚄 the views!!',                  now() - interval '4 days' + interval '5 minutes'),
    (conv_japan, me_id,  'Looks amazing Sara 🙏 60 days!!! 🇯🇵',                     now() - interval '6 hours');
